...
public:
    ...
    XPathResultSet( const XPathQuery &query )
        : query_{query}, resultSet_{query_.query_.get( )->nodesetval},
          sz_{resultSet_ ? ( size_t )resultSet_->nodeNr : 0}, idx_{0} {} // ! nodeNr >= 0
    XPathResultSet( const XPathResultSet &rs )
        : query_{rs.query_}, resultSet_{query_.query_.get( )->nodesetval}, sz_{rs.sz_},
          idx_{0} {}
    XPathResultSet( XPathResultSet &&rhs )
        : query_{std::move( rhs.query_ )}, resultSet_{query_.query_.get( )->nodesetval},
          sz_{rhs.sz_}, idx_{0} {
    ...
    auto operator=( XPathResultSet &&rhs ) -> XPathResultSet & {
        ...
        resultSet_ = query_.query_.get( )->nodesetval;
        sz_ = rhs.sz_;
        idx_ = rhs.idx_;
        rhs.resultSet_ = nullptr;
        rhs.sz_ = rhs.idx_ = 0;
